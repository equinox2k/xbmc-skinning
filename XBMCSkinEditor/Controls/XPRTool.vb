...
Public Class XPRTool
    ...
    Private colXPRImages As New Collection
    Private colXPRFileInfo As New Collection
    ...
    Public ReadOnly Property FileCount() As Integer
        Get
            Return colXPRFileInfo.Count
        End Get
    End Property

    Public ReadOnly Property FileName(ByVal Index As Integer) As String
        Get
            Return CType(colXPRFileInfo.Item(Index), XPRFileInfo).Name
        End Get
    End Property

    Public ReadOnly Property GetImage(ByVal Index As Integer) As SkinImage
        Get
            Dim objXPRFileInfo As XPRFileInfo = CType(colXPRFileInfo.Item(Index), XPRFileInfo)
    ...
    Public ReadOnly Property GetImage(ByVal FileName As String) As SkinImage
        Get
            Dim objXPRFileInfo As XPRFileInfo = CType(colXPRFileInfo.Item(FileName), XPRFileInfo)
            objBinaryReader.BaseStream.Seek(objXPRFileInfo.Offset, SeekOrigin.Begin)
            Dim bytPackedFile() As Byte = objBinaryReader.ReadBytes(objXPRFileInfo.PackedSize)
            Dim bytUnpackedFile() As Byte = Decompress(objXPRFileInfo.UnpackedSize, bytPackedFile)
            Return ConvertTextures(bytUnpackedFile)
    ...
    Public Sub OpenXPR(ByVal FileName As String)
        ...
        For intloop As Integer = 1 To (intHeaderSize - 12) >> 7
            ...
            colXPRFileInfo.Add(objXPRFileInfo, objXPRFileInfo.Name)
        Next intloop
    ...
    Private Function ConvertTextures(ByVal Buffer() As Byte) As SkinImage

        Dim objSkinImage As New SkinImage
    ...
    Private Function ConvertTextures(ByVal SkinImage As SkinImage, ByVal HasPalette As Boolean, ByVal RealSizeWidth As Integer, ByVal RealSizeHeight As Integer, ByVal Textures As Collection, ByVal ResDataOffset As Integer, ByVal Buffer() As Byte) As SkinImage
        ...
        For intLoop As Integer = 1 To Textures.Count
            ...
            If HasPalette = False Then
                ...
                Dim objImageBack As New Bitmap(CInt(intTexWidth), CInt(RealSizeHeight), PixelFormat.Format32bppArgb)
                Dim objGraphics As Graphics = Graphics.FromImage(objImageBack)
                objGraphics.Clear(Color.Transparent)

                Dim m_BitmapData As BitmapData
                Dim objBitmap As New Bitmap(objImageBack)
                Dim bounds As Rectangle = New Rectangle(0, 0, objBitmap.Width, objBitmap.Height)
                m_BitmapData = objBitmap.LockBits(bounds, Imaging.ImageLockMode.ReadWrite, Imaging.PixelFormat.Format32bppArgb)
                ...
                objBitmap = objBitmap.Clone(New Rectangle(0, 0, RealSizeWidth, RealSizeHeight), PixelFormat.Format32bppArgb)
